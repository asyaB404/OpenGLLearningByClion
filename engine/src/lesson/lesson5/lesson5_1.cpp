...
// ============================================================================
// 渲染管线与本课代码的对应关系（从 CPU 到屏幕）
// ============================================================================
// 【应用阶段 · CPU】在 glDrawArrays 之前完成：
//   - 几何：vertices → VBO；VAO 记录顶点属性布局（location 0 位置，1 纹理坐标）。
//   - 程序与纹理：Shader 链接着色器；纹理绑到 GL_TEXTURE0/1，setInt 关联 sampler2D。
//   - 每帧：glClear（含深度缓冲）；绑定纹理；ourShader.use()；setMat4 传入 MVP。
//   - 绘制命令：glBindVertexArray + glDrawArrays 把「画 36 个顶点构成的三角形」交给 GPU。
//
// 【GPU 管线】（Core Profile 下与本课直接相关的部分）：
//   1. 输入装配：按 VAO 从 VBO 读出每个顶点的属性，送入顶点着色器。
//   2. 顶点着色器（5.1.coordinate_systems.vs）：
//      gl_Position = projection * view * model * vec4(aPos, 1.0)；// 局部→世界→观察→裁剪
//      TexCoord = aTexCoord；// 传给后续阶段（经光栅化插值到每个片段）。
//   3. 图元装配与裁剪：36 顶点组成三角形；裁剪空间内裁剪，透视除法，映射到视口像素。
//   4. 光栅化：三角形覆盖的每个像素生成片段，顶点输出（如 TexCoord）在三角形内插值。
//   5. 片段着色器（5.1.coordinate_systems.fs）：按插值 TexCoord 采样 texture1/2，mix 得颜色。
//   6. 逐片段操作：深度测试（本课 glEnable(GL_DEPTH_TEST)）等，通过则写入颜色缓冲。
//
// 【显示】glfwSwapBuffers：前后台缓冲交换，最终图像显示在窗口。
// ============================================================================
...
int lesson5_1_main()
{
    ...
    // ========================================================================
    // 第八步：渲染循环（每帧重复「应用阶段 → glDrawArrays → GPU 管线」）
    // 详细阶段说明见文件顶部「渲染管线与本课代码的对应关系」注释块。
    // ========================================================================
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            // 绘制立方体（36 个顶点）。自此进入 GPU 管线：顶点着色器(MVP 与 TexCoord)
            // → 装配/裁剪/视口 → 光栅化(插值) → 片段着色器(纹理与颜色) → 深度测试 → 写颜色缓冲
