// ============================================================================
// Lesson 8.2: 基础光照（Basic Lighting）- Phong 光照模型 + 动态光源
// ============================================================================
// 本课程学习内容：
...
// ============================================================================
// Lesson8_2Application 类 - 继承自 CameraApplication
// ============================================================================
class Lesson8_2Application : public CameraApplication
{
public:
    Lesson8_2Application() 
        : CameraApplication(800, 600, "OpenGL Learning - Lesson 8.2: Basic Lighting (Phong + Moving Light)")
    {
    }
...
protected:
    ...
    virtual void OnUpdate(float deltaTime) override
    {
        ...
        // 更新光源位置（随时间移动）
        // 使用 sin 函数让光源在 X 和 Y 轴上周期性移动
        float currentTime = GetTime();
        m_lightPos.x = 1.0f + sin(currentTime) * 2.0f;        // X 轴：在 -1 到 3 之间移动
        m_lightPos.y = sin(currentTime / 2.0f) * 1.0f;      // Y 轴：在 -1 到 1 之间移动（速度是 X 轴的一半）
        // m_lightPos.z 保持不变（2.0f）
    }
    ...
    virtual void OnRender() override
    {
        ...
        // 注意：光源位置在 OnUpdate 中已经更新，所以光源立方体会跟随移动
        model = glm::mat4(1.0f);
...
private:
    ...
    glm::vec3 m_lightPos = glm::vec3(1.2f, 1.0f, 2.0f);  // 光源位置（初始值，会在 OnUpdate 中更新）
};
